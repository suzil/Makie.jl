...
function to_absolute(canvas::Canvas{2}, x::Relative{T}) where T
    minimum(widths(canvas.cam_area)) .* T(x)
end
function to_absolute(canvas::Canvas{2}, x::Pixel{T}) where T
    T(x) .* minimum(Makie.wscale(canvas.pixel, canvas.cam_area))
end
function to_absolute(canvas::Canvas{2}, x::Millimeter{T}) where T
    to_absolute(canvas, to_px.(x)) .* pixel2mm(canvas)
end

function to_absolute(canvas::Canvas{2}, x::Vec{2, RT}) where {RT <: Relative{T}} where T
    Vec(widths(canvas.cam_area) .* T.(x))
end
function to_absolute(canvas::Canvas{2}, x::Vec{2, PT}) where {PT <: Pixel{T}} where T
    Vec(T.(x) .* Makie.wscale(canvas.pixel, canvas.cam_area))
end
function to_absolute(canvas::Canvas{2}, p::Vec{2, PT}) where {PT <: Millimeter{T}} where T
    ppx = to_absolute(canvas, to_px.(p)) .* pixel2mm(canvas)
    Vec(ppx)
end

function to_absolute(canvas::Canvas{2}, x::Point{2, RT}) where {RT <: Relative{T}} where T
    p = T.(x)
    cam_area = canvas.cam_area
    Point(widths(cam_area) .* p) .+ Point(minimum(cam_area))
end
function to_absolute(canvas::Canvas{2}, x::Point{2, PT}) where {PT <: Pixel{T}} where T
    cs = Makie.camspace(canvas.cam_area, canvas.pixel, T.(x))
    Point(cs)
end
function to_absolute(canvas::Canvas{2}, p::Point{2, PT}) where {PT <: Millimeter{T}} where T
